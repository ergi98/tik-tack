...
const MainScreen: React.FC<MainScreenProps> = ({ playerId }) => {
...
const OpponentSelector: React.FC<OpponentSelectorProps> = ({
  ...
  const getIcon = (key: string) => {
    ...
    if (key === OPPONENTS.ONLINE) return <Globe />;
    return null;
  };
  ...
  const sliderPosition =
    opponent === OPPONENTS.PVP
      ? "left-0"
      : opponent === OPPONENTS.ONLINE
      ? "right-0"
      : "";

  const sliderStyles =
    opponent === OPPONENTS.PVC
      ? { left: optionRef.current?.clientWidth + "px" }
      : {};
  ...
  return (
    ...
    >
      <motion.div
        layout
        style={sliderStyles}
        className={`opponent-slider absolute w-1/3 h-full top-0 bg-neutral-600 ${sliderPosition}`}
      />
      {Object.keys(OPPONENTS).map((key, i) => (
        <motion.div
          key={key}
          className="z-10"
